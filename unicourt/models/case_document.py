...
class CaseDocument(BaseModel):
    ...
    CaseDocument
    """ # noqa: E501
    object: Annotated[str, Field(min_length=12, strict=True, max_length=12)] = Field(description="Name of the object")
    case_document_id: Annotated[str, Field(min_length=18, strict=True, max_length=18)] = Field(description="UniCourt's Case Document ID.", alias="caseDocumentId")
    sort_order: Optional[StrictInt] = Field(description="Order of documents how it is being stored in UniCourt.", alias="sortOrder")
    name: Optional[Annotated[str, Field(strict=True, max_length=255)]] = Field(description="Document name.")
    description: Optional[StrictStr] = Field(description="Description of the document.")
    document_filed_date: Optional[Annotated[str, Field(min_length=25, strict=True, max_length=25)]] = Field(description="Document Date when it was filed. The document date will be either explicitly provided byt the court or if the document is associatated to a docket entry then the document date will be of docket entry date.", alias="documentFiledDate")
    parent_document_id: Optional[Annotated[str, Field(min_length=18, strict=True, max_length=18)]] = Field(description="Document ID which is the parent document for the current document. This will be null if the current document is a parent document.", alias="parentDocumentId")
    child_document_id_array: List[Optional[Annotated[str, Field(min_length=18, strict=True, max_length=18)]]] = Field(description="List of child document ID's if exists or else it will be an empty lsit.", alias="childDocumentIdArray")
    pages: Optional[StrictInt] = Field(description="Total number of pages in the document.")
    is_preview_available: StrictBool = Field(description="Determines if a preview is available for the case document.", alias="isPreviewAvailable")
    preview_document: PreviewDocument = Field(alias="previewDocument")
    price: Optional[Union[StrictFloat, StrictInt]] = Field(description="Price of the document.")
    in_library: StrictBool = Field(description="Determines if the document is present in the UniCourt Library or not.", alias="inLibrary")
    added_to_library_date: Optional[Annotated[str, Field(min_length=25, strict=True, max_length=25)]] = Field(description="Date and time when the document was downloaded and added to the UniCourt Crowd Source Library.", alias="addedToLibraryDate")
    estimated_order_duration: Annotated[str, Field(min_length=10, strict=True, max_length=25)] = Field(description="Estimated duration of a Order.", alias="estimatedOrderDuration")
    download_api: Optional[Annotated[str, Field(strict=True, max_length=255)]] = Field(description="Link to either view the document if it is downloaded and already present in the UniCourt CrowdSourced Library.", alias="downloadAPI")
    first_fetch_date: Annotated[str, Field(min_length=25, strict=True, max_length=25)] = Field(description="Is the date when the document was first fetched from the court site.", alias="firstFetchDate")
    source_data_status: Optional[StrictStr] = Field(description="The status of source data of document. If the value of sourceDataStatus is SOURCE_DEPRECATED then it means that the Document has been migrated from old court site to a new court site and the data being shown in the API response is from a old court site. If the sourceDataStatus is NO_LONGER_AVAILABLE_IN_COURT then it means that a particular case is invalid in the court site.", alias="sourceDataStatus")
