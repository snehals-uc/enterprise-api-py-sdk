...
class Case(BaseModel):
    ...
    Case
    """ # noqa: E501
    object: Annotated[str, Field(min_length=4, strict=True, max_length=4)] = Field(description="Name of the object")
    case_id: Annotated[str, Field(min_length=18, strict=True, max_length=18)] = Field(description="Unique identifier of Case.", alias="caseId")
    url: Optional[Annotated[str, Field(strict=True, max_length=500)]] = Field(description="URL to the case page in UniCourt Application.")
    case_number: Annotated[str, Field(min_length=3, strict=True, max_length=50)] = Field(description="Case number as provided by Court.", alias="caseNumber")
    case_name: Optional[Annotated[str, Field(strict=True, max_length=250)]] = Field(description="Case name as provided by Court.", alias="caseName")
    filed_date: datetime = Field(description="Filing date for the case provided by the Court. Formatted as YYYY-MM-DDTHH:MM:SS+ZZ:zz", alias="filedDate")
    has_only_meta_info: StrictBool = Field(description="This field determines if this case object has only meta information. If the value is true and if the full information is required you would need to call the updateCase API.", alias="hasOnlyMetaInfo")
    court_service_status_id: Optional[Annotated[str, Field(min_length=18, strict=True, max_length=18)]] = Field(description="Court Service Status ID of the requested case where we can use it to get the service status", alias="courtServiceStatusId")
    court_service_status_api: Optional[Annotated[str, Field(strict=True, max_length=250)]] = Field(description="API to get the service statuses of the given case.", alias="courtServiceStatusAPI")
    court: Court
    court_location: CourtLocation = Field(alias="courtLocation")
    case_type: CaseType = Field(alias="caseType")
    charge_array: List[CaseCharge] = Field(description="Array of charges that are added to this case.", alias="chargeArray")
    case_status: CaseStatus = Field(alias="caseStatus")
    cause_of_action_array: List[CaseCauseOfAction] = Field(description="Array of cause of Actions that are added to this case.", alias="causeOfActionArray")
    first_fetch_date: datetime = Field(description="The date and time when the case was first fetched from the Court. This date and time is in UTC. Formatted as YYYY-MM-DDTHH:MM:SS+ZZ:zz,.", alias="firstFetchDate")
    last_fetch_date: datetime = Field(description="The date and time when the case was last fetched from the Court. This date and time is in UTC. Formatted as YYYY-MM-DDTHH:MM:SS+ZZ:zz, Note: It is not necessary that every time the case is fetched from Court we find changes in the case information. It could be that we already have the latest information from the Court and no changes exist.", alias="lastFetchDate")
    last_fetch_date_with_updates: datetime = Field(description="The date and time when the case was last fetched from the Court where we found changes in the case information. This date and time is in UTC. Formatted as YYYY-MM-DDTHH:MM:SS+ZZ:zz,", alias="lastFetchDateWithUpdates")
    ...
    source_data_status: Optional[StrictStr] = Field(description="The status of source data of case. If the value of sourceDataStatus is SOURCE_DEPRECATED then it means that the Case has been migrated from old court site to a new court site and the data being shown in the API response is from a old court site. If the sourceDataStatus is NO_LONGER_AVAILABLE_IN_COURT then it means that a particular case is invalid in the court site.", alias="sourceDataStatus")
    source_case_data: SourceCaseData = Field(alias="sourceCaseData")
    has_documents_with_preview: StrictBool = Field(description="This field will be set to TRUE if atleast one document has a preview.", alias="hasDocumentsWithPreview")
    export_api: Annotated[str, Field(strict=True, max_length=255)] = Field(description="When a case is beyond the threshold of entities we provide this link so that the user can request and get all the data of the case with one additional call. This data will be zipped and sent via a webhoook.", alias="exportAPI")
