...
class CourtCoverage(BaseModel):
    ...
    CourtCoverage
    """ # noqa: E501
    object: Annotated[str, Field(min_length=13, strict=True, max_length=13)] = Field(description="Name of the object")
    last_update_count_date: Annotated[str, Field(min_length=25, strict=True, max_length=25)] = Field(description="Date when it was last updated.", alias="lastUpdateCountDate")
    court: Court
    total_case_count: StrictInt = Field(description="Total Cases for a specific court.", alias="totalCaseCount")
    total_cases_in_last_thirty_days_count: StrictInt = Field(description="Total Cases in last 30 days that were added to UniCourt", alias="totalCasesInLastThirtyDaysCount")
    total_free_case_document_count: StrictInt = Field(description="Total Free Case Documents for a specific court.", alias="totalFreeCaseDocumentCount")
    total_free_case_documents_in_last_thirty_days_count: StrictInt = Field(description="Total Free Case Documents in last 30 days that were added to UniCourt", alias="totalFreeCaseDocumentsInLastThirtyDaysCount")
    total_paid_case_document_count: StrictInt = Field(description="Total Paid Case Documents for a specific court.", alias="totalPaidCaseDocumentCount")
    total_paid_case_documents_in_last_thirty_days_count: StrictInt = Field(description="Total Paid Case Documents in last 30 days that were added to UniCourt", alias="totalPaidCaseDocumentsInLastThirtyDaysCount")
    total_case_document_in_library_count: StrictInt = Field(description="Count of total Case Documents added in UniCourt Library.", alias="totalCaseDocumentInLibraryCount")
    total_case_document_in_library_in_last_thirty_days_count: StrictInt = Field(description="Count of total Case Documents added in UniCourt Library in last 30 days", alias="totalCaseDocumentInLibraryInLastThirtyDaysCount")
    case_class_coverage_array: List[CaseClassCoverage] = Field(alias="caseClassCoverageArray")
