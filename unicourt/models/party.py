...
class Party(BaseModel):
    ...
    Party
    """ # noqa: E501
    object: Annotated[str, Field(min_length=5, strict=True, max_length=5)] = Field(description="Name of the object")
    party_id: Annotated[str, Field(min_length=18, strict=True, max_length=18)] = Field(description="ID for the party in this case. This ID is unique within a case and NOT across cases. If the same party were to appear in another case this ID would be different.", alias="partyId")
    name: Annotated[str, Field(min_length=1, strict=True, max_length=250)] = Field(description="Name of the party as provided by Court.")
    name_prefix: Optional[Annotated[str, Field(strict=True, max_length=10)]] = Field(alias="namePrefix")
    first_name: Optional[Annotated[str, Field(strict=True, max_length=250)]] = Field(description="First name of the party. This is normalized by UniCourt.", alias="firstName")
    middle_name: Optional[Annotated[str, Field(strict=True, max_length=100)]] = Field(description="Middle name of the party. This is normalized by UniCourt.", alias="middleName")
    last_name: Optional[Annotated[str, Field(strict=True, max_length=100)]] = Field(description="Last name of the party. This is normalized by UniCourt.", alias="lastName")
    name_suffix: Optional[Annotated[str, Field(strict=True, max_length=10)]] = Field(alias="nameSuffix")
    contact: Contact
    party_classification_type: Annotated[str, Field(strict=True, max_length=50)] = Field(description="To know the type of an entity in a case, if it is an Individual, Company or Other. An entity to a case could be the parties, attorneys or judges involved.", alias="partyClassificationType")
    party_role: PartyRole = Field(alias="partyRole")
    source_party_role: Annotated[str, Field(strict=True, max_length=100)] = Field(description="Party Type as provided by Court.", alias="sourcePartyRole")
    is_visible: StrictBool = Field(description="Signifies if the party as this party type is currently isVisible or not for the case.", alias="isVisible")
    first_fetch_date: datetime = Field(description="When was the party first fetched from the court site.", alias="firstFetchDate")
    last_fetch_date: datetime = Field(description="When was the party last fetched from the court site.", alias="lastFetchDate")
    ...
    possible_norm_party_array: List[PossibleNormParty] = Field(alias="possibleNormPartyArray")
