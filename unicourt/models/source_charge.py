...
class SourceCharge(BaseModel):
    ...
    source_charge_raw: Annotated[str, Field(strict=True, max_length=255)] = Field(description="Raw charge data from the source site.", alias="sourceChargeRaw")
    source_charge: Optional[Annotated[str, Field(strict=True, max_length=255)]] = Field(description="Charge data from the source site.", alias="sourceCharge")
    is_visible: StrictBool = Field(description="Signifies if the charge is currently isVisible or not for the case.", alias="isVisible")
    source_statute: Optional[Annotated[str, Field(strict=True, max_length=50)]] = Field(description="Statute of a charge.", alias="sourceStatute")
    source_charge_degree: Optional[Annotated[str, Field(strict=True, max_length=255)]] = Field(description="Charge degree data from the source site.", alias="sourceChargeDegree")
    source_charge_severity: Optional[Annotated[str, Field(strict=True, max_length=255)]] = Field(description="Charge severity data from the source site.", alias="sourceChargeSeverity")
    source_charge_additional_data_array: List[SourceChargeAdditionalData] = Field(description="Additional data related to the charge which is available in the source site.", alias="sourceChargeAdditionalDataArray")
    first_fetch_date: Annotated[str, Field(min_length=25, strict=True, max_length=25)] = Field(description="When this charge was first fetched from the court site.", alias="firstFetchDate")
    last_fetch_date: Annotated[str, Field(min_length=25, strict=True, max_length=25)] = Field(description="When this charge was last fetched from the court site.", alias="lastFetchDate")
