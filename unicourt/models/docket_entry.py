...
class DocketEntry(BaseModel):
    ...
    sort_order: StrictInt = Field(description="Order number how the dockets have stored in UniCourt.", alias="sortOrder")
    docket_entry_date: datetime = Field(description="Docket Action Date", alias="docketEntryDate")
    docket_number: Optional[StrictInt] = Field(description="The respective docket entry number which is defined in the court website.", alias="docketNumber")
    docket_badge: StrictStr = Field(description="Docket Badge helps you to know what type of a docket entry it is.", alias="docketBadge")
    text: StrictStr = Field(description="Source Docket Entry")
    text_structured: SourceStructuredData = Field(alias="textStructured")
    referenced_docket_number_array: List[ReferencedDocketNumber] = Field(description="Other Docket Numbers that referenced for a particular docket entry.", alias="referencedDocketNumberArray")
    ...
    last_fetch_date: Annotated[str, Field(min_length=25, strict=True, max_length=25)] = Field(description="When this docket entry was last fetched from the source.", alias="lastFetchDate")
    boundary: Optional[StrictStr] = Field(description="Determines if it is the first docket entry or the last docket entry. This value will be set only for the first and last docket entry. For other docket entries it will be null. However, this will be set as single_docket_entry when the Case contains only one docket entry.")
