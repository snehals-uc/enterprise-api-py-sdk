...
class SOSData(BaseModel):
    ...
    SOSData
    """ # noqa: E501
    object: Annotated[str, Field(strict=True, max_length=7)]
    sos_number: Optional[Annotated[str, Field(strict=True, max_length=50)]] = Field(alias="sosNumber")
    state_code: Annotated[str, Field(strict=True, max_length=2)] = Field(alias="stateCode")
    fein: Optional[Annotated[str, Field(strict=True, max_length=16)]]
    domestic_registration: StrictBool = Field(alias="domesticRegistration")
    registered_date: Optional[datetime] = Field(alias="registeredDate")
    status: Optional[Annotated[str, Field(strict=True, max_length=10)]]
    is_active: StrictBool = Field(alias="isActive")
    inactivation_date: Optional[datetime] = Field(alias="inactivationDate")
    associated_so_s_person_array: List[AssociatedSoSPerson] = Field(alias="associatedSoSPersonArray")
    contact: Contact
    name_changes_array: List[SOSNameChange] = Field(alias="nameChangesArray")
    sos_associated_norm_organization_array: List[SOSAssociatedNormOrganization] = Field(alias="sosAssociatedNormOrganizationArray")
    first_fetch_date: datetime = Field(alias="firstFetchDate")
    last_fetch_date: datetime = Field(alias="lastFetchDate")
    last_fetch_date_with_updates: datetime = Field(description="Last Fetch Date of Organization with Updates.", alias="lastFetchDateWithUpdates")
